...
11. 当前阶段验收完成后执行知识复盘筛选；若存在高价值内容，在最终报告中整理实际问题和相关面试知识点，否则省略整个复盘部分。

### 7.1 Phase 完成后的知识复盘

知识复盘仅在当前 Phase 的必需验收完成后执行，并直接随完成报告发给用户。它不改变阶段状态，也不是进入下一阶段的替代验收项。

**有价值的问题**必须来自本阶段实际开发过程，并按以下结构整理：

1. 问题现象：可观察到的错误、风险或验收阻塞。
2. 根本原因：造成问题的技术或流程原因。
3. 解决方法：本阶段实际采用并验证过的处理方式。
4. 后续预防：可复用到后续 Phase 的检查、测试或设计约束。

**面试知识点**必须与本阶段实现直接相关，并按以下结构整理：

1. 典型问题：面试中可能出现的具体提问。
2. 回答要点：原理、取舍和关键边界。
3. 项目实践：对应到 AgentHub 的代码、测试或决策，确保能够结合真实项目说明。

每类默认最多 5 条并按价值排序。不得为了填满数量收录通用八股文、一次性低价值失误或未经验证的猜测。若两类均无高价值条目，最终报告中不得出现知识复盘标题或“暂无内容”等占位语。
...
8. 通用约束包含 Phase 完成后的条件式知识复盘要求，且明确无高价值内容时不输出占位部分。
